...
export type ProduceResponse = {
    responses: (({ name: string } | { topicId: string }) & {
        partitionResponses: {
            index: number;
            errorCode: number;
            baseOffset: bigint;
...
export const throwIfError = <T extends ProduceResponse>(result: T) => {
...
export const createBatch = (partition: ProduceRequest['topicData'][0]['partitionData'][0]) => {
