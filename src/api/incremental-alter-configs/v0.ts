...
import { IncrementalAlterConfigsRequest, IncrementalAlterConfigsResponse, throwIfError } from './common';
...
IncrementalAlterConfigs Response (Version: 0) => { throttle_time_ms [responses] }
  ...
  responses => { error_code error_message resource_type resource_name }
    ...
    resource_name => STRING
*/
export const INCREMENTAL_ALTER_CONFIGS_V0 = createApi<IncrementalAlterConfigsRequest, IncrementalAlterConfigsResponse>({
    apiKey: 44,
    apiVersion: 0,
            ...
            .writeArray(data.resources, (encoder, resource) =>
                encoder
                    ...
                    .writeArray(resource.configs, (encoder, config) =>
                        encoder.writeString(config.name).writeInt8(config.configOperation).writeString(config.value),
                    ),
            )
